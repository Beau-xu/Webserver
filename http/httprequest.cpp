...
bool HttpRequest::parse(std::string& buffer) {
    ...
    auto itRead = buffer.cbegin();
    auto itEnd = buffer.cend();
    while (itEnd - itRead <= 2 && state_ != FINISH) {
        auto itLineEnd = std::search(itRead, itEnd, CRLF, CRLF + 2);
        std::string line(itRead, itLineEnd);
        switch (state_) {
            case REQUEST_LINE:
...
void HttpRequest::parseFromUrlencoded_() {  // 获取POST报文体
...
bool HttpRequest::userVerify(const std::string& name, const std::string& pwd, bool isLogin) {
    ...
    MYSQL* sql;
    SqlConnection(&sql, SqlConnPool::instance());
    assert(sql);
